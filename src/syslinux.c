...
BOOL InstallSyslinux(DWORD drive_index, char drive_letter, int fs_type)
{
	...
	if ((syslinux_ldlinux_len[0] != 0) && (syslinux_ldlinux_len[1] != 0)) {
		...
		for (i=0; i<2; i++) {
			...
			//uprintf("Using existing './%s' %s", path, IsBufferInDB(syslinux_ldlinux[i], (size_t)syslinux_ldlinux_len[i])?"✓":"✗");
		}
	} else {
		for (i=0; i<2; i++) {
	...
	if (bt == BT_SYSLINUX_V6) {
		...
		if (fd == NULL) {
			...
			if (CopyFileU(&path[3], path, TRUE)) {
				//uprintf("Created '%s' (from '%s/%s-%s/%s') %s", path, FILES_DIR, syslinux,	embedded_sl_version_str[1], &path[3], IsFileInDB(&path[3])?"✓":"✗");
			} else {
