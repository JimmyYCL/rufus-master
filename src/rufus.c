...
//Jimmy:20170628 add file name info 
HWND hFileName;
...
static void SetToGo(void)
{
  ...
  //Jimmy:20170626 disable reposition
...
static void EnableControls(BOOL bEnable)
{
	...
	//Jimmy:20170628 add file name info 
	EnableWindow(GetDlgItem(hMainDialog, IDC_FILENAME_INFO), bEnable);
}
...
//Jimmy:20170628 add file name info 
//
// Print file name to the text tag
//
static void PrintFileNameMessage(char* msg) {
	SetWindowTextU(hFileName, msg);
}
...
DWORD WINAPI ISOScanThread(LPVOID param)
{
	...
	if (!IS_DD_BOOTABLE(img_report) && !IS_BIOS_BOOTABLE(img_report) && !IS_EFI_BOOTABLE(img_report)) {
		...
		if (!dont_display_image_name) {
			...
			//Jimmy:20170628 display file name on the text tag
			PrintFileNameMessage(&image_path[i + 1]);
...
static void InitDialog(HWND hDlg)
{
	...
	//Jimmy:20170628 add file name info 
	hFileName = GetDlgItem(hDlg, IDC_FILENAME_INFO);
  ...
  // Jimmy:20170626 disable language toolbar
  ...
  //Jimmy:20170626 do not reposition advanced button 
  ...
  //Jimmy:20170626 do not reposition image button 
  ...
  //Jimmy:20170626 disable reposition since there is no advanced controls
...
//Jimmy:20170626 disable language
...
//			SelectionDyn("Test Choice", "Unused", choices, ARRAYSIZE(choices));
	...
	case WM_INITDIALOG:
	  ...
	  //Jimmy:20170623 disable localization
		//apply_localization(IDD_DIALOG, hDlg);
		
		//Jimmy:20170623 disable update check to prevent pop up update dialog
		//SetUpdateCheck();
